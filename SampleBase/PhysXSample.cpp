...
{
	physx::debugger::comm::PvdConnectionManager* pvd = mPhysics->getPvdConnectionManager();
	if(!pvd)
		return;
	...
	PVD::PvdConnection* theConnection = PxVisualDebuggerExt::createConnection(pvd, mPvdParams.ip, mPvdParams.port, mPvdParams.timeout, theConnectionFlags);
	if(theConnection)
	...
	if(mCreateCudaCtxManager)
	{
		...
		if (!mApplication.getCommandLine().hasSwitch("nointerop"))
		{
			...
			case Renderer::DRIVER_DIRECT3D9:
				cudaContextManagerDesc.interopMode = pxtask::CudaInteropMode::D3D9_INTEROP;
				break;
			...
			case Renderer::DRIVER_OPENGL:
				cudaContextManagerDesc.interopMode = pxtask::CudaInteropMode::OGL_INTEROP;
				break;
			};
...
void PhysXSample::collectInputEvents(std::vector<const SampleFramework::InputEvent*>& inputEvents)
{
	...
	DIGITAL_INPUT_EVENT_DEF(SPAWN_DEBUG_OBJECT,				WKEY_1,			XKEY_1,			PS3KEY_1,		AKEY_UNKNOWN,	OSXKEY_1,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_1,			WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(PAUSE_SAMPLE,					WKEY_P,			XKEY_P,			PS3KEY_P,		AKEY_UNKNOWN,	OSXKEY_P,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_P,			WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(STEP_ONE_FRAME,					WKEY_O,			XKEY_O,			PS3KEY_O,		AKEY_UNKNOWN,	OSXKEY_O,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_O,			WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(TOGGLE_VISUALIZATION,			WKEY_V,			XKEY_V,			PS3KEY_V,		AKEY_UNKNOWN,	OSXKEY_V,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_V,			WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(DECREASE_DEBUG_RENDER_SCALE,	WKEY_F7,		XKEY_F7,		PS3KEY_F7,		AKEY_UNKNOWN,	OSXKEY_F7,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_F7,		WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(INCREASE_DEBUG_RENDER_SCALE,	WKEY_F8,		XKEY_F8,		PS3KEY_F8,		AKEY_UNKNOWN,	OSXKEY_F8,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_F8,		WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(HIDE_GRAPHICS,					WKEY_G,			XKEY_G,			PS3KEY_G,		AKEY_UNKNOWN,	OSXKEY_G,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_G,			WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(WIREFRAME,						WKEY_N,			XKEY_N,			PS3KEY_N,		AKEY_UNKNOWN,	OSXKEY_N,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_N,			WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(CCD,							WKEY_I,			XKEY_I,			PS3KEY_I,		AKEY_UNKNOWN,	OSXKEY_I,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_I,			WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(TOGGLE_PVD_CONNECTION,			WKEY_F5,		XKEY_F5,		PS3KEY_F5,		AKEY_UNKNOWN,	OSXKEY_F5,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_F5,		WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(SHOW_HELP,						WKEY_H,			XKEY_H,			PS3KEY_H,		AKEY_UNKNOWN,	OSXKEY_H,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_H,			WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(SHOW_DESCRIPTION,				WKEY_F,			XKEY_F,			PS3KEY_F,		AKEY_UNKNOWN,	OSXKEY_F,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_F,			WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(VARIABLE_TIMESTEP,				WKEY_T,			XKEY_T,			PS3KEY_T,		AKEY_UNKNOWN,	OSXKEY_T,		PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_T,			WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(NEXT_PAGE,						WKEY_NEXT,		XKEY_UNKNOWN,	PS3KEY_NEXT,	AKEY_UNKNOWN,	OSXKEY_RIGHT,	PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_NEXT,		WIIUKEY_UNKNOWN		);
	DIGITAL_INPUT_EVENT_DEF(PREVIOUS_PAGE,					WKEY_PRIOR,		XKEY_UNKNOWN,	PS3KEY_PRIOR,	AKEY_UNKNOWN,	OSXKEY_LEFT,	PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_PRIOR,		WIIUKEY_UNKNOWN		);	
	DIGITAL_INPUT_EVENT_DEF(PROFILE_ONLY_PVD,				WKEY_9,			XKEY_UNKNOWN,	PS3KEY_UNKNOWN,	AKEY_UNKNOWN,	OSXKEY_UNKNOWN,	PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_UNKNOWN,	WIIUKEY_UNKNOWN		);
	//DIGITAL_INPUT_EVENT_DEF(PAUSE_SAMPLE,					WKEY_P,			XKEY_UNKNOWN,	PS3KEY_UNKNOWN,	AKEY_UNKNOWN,	OSXKEY_UNKNOWN,	PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_UNKNOWN);
	//DIGITAL_INPUT_EVENT_DEF(STEP_ONE_FRAME,					WKEY_O,			XKEY_UNKNOWN,	PS3KEY_UNKNOWN,	AKEY_UNKNOWN,	OSXKEY_UNKNOWN,	PSP2KEY_UNKNOWN,	IKEY_UNKNOWN,	LINUXKEY_UNKNOWN);

	//digital gamepad events
	DIGITAL_INPUT_EVENT_DEF(SHOW_HELP,						GAMEPAD_SELECT,	GAMEPAD_SELECT,	GAMEPAD_SELECT,	AKEY_UNKNOWN,	GAMEPAD_SELECT,	GAMEPAD_SELECT,	IKEY_UNKNOWN,	LINUXKEY_UNKNOWN,	GAMEPAD_SELECT);
	DIGITAL_INPUT_EVENT_DEF(SPAWN_DEBUG_OBJECT,				GAMEPAD_NORTH,	GAMEPAD_NORTH,	GAMEPAD_NORTH,	AKEY_UNKNOWN,	GAMEPAD_NORTH,	GAMEPAD_NORTH,	IKEY_UNKNOWN,	LINUXKEY_UNKNOWN,	GAMEPAD_NORTH);
...
PxRigidDynamic* PhysXSample::createTestCompound(const PxVec3& pos, PxU32 nbBoxes, float boxSize, float amplitude, const PxVec3* vel, RenderMaterial* material, PxReal density, bool makeSureVolumeEmpty)
{
	...
	if (makeSureVolumeEmpty)
	{
		...
		PxOverlapBuffer buf;
		if (getActiveScene().overlap(geometry, pose, buf))
		{
