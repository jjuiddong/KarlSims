	...
	RendererShape(renderer)
{
	...
	const PxU32 numVerts = NODE_COUNT*CONER_COUNT;
	...
	const PxU32 numIndices = (6*CONER_COUNT) * (NODE_COUNT-1) + ((CONER_COUNT-2) * 6);
	...
	if (m_indexBuffer)
	{
		...
		if (indices)
		{
			...
			// back
			const int lastVtxIdx = numVerts-CONER_COUNT;
			for (int k=0; k < CONER_COUNT-2; ++k)
			{
				indices[ writeStartIndiceIdx++] = lastVtxIdx;
				indices[ writeStartIndiceIdx++] = lastVtxIdx+k+1;
				indices[ writeStartIndiceIdx++] = lastVtxIdx+k+2;
			}
